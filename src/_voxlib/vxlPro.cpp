...
template<typename T>  bool reset( stringstream& ins, voxelImageT<T>& vImg)  {
	...
	ins >>param;
	while(param.size())  {
		if(param=="maxNz")  { ins>>maxNz;  cout<<"maxNz:"<<maxNz<<" "; maxNz=std::min(maxNz,_maxNz); } // stop reading after maxNz layers
...
template<typename T> VxlFuncs<T> namedProcesses() requires(sizeof(T)<=2) {
	...
	#ifdef LPNG
		{  "sliceToPng"        , sliceToPng },
		{  "sliceToPngBW"      , sliceToPngBW },
	#endif
		{  "operation"         , operat  },
		{  "operat"            , operat }
	};

	addFuncs<T>(key_funs); //EXP:;
	return key_funs;
}
...
template<typename T> VxlFuncs<T> namedProcesses() requires(sizeof(T)>=3) {
	...
	#endif
	auto key_funs = VxlFuncs<T>{
		{  ""          , raiseErr},// ProcessP can be removed if using g++
		{  ";"         , raiseErr }, // TODO delete
		{  "skip"      , ignore },
		{  "Offset"    , Offset },
		{  "redirect"  , redirect },
